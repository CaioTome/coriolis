...
* **Capítulo 12 (Recursos do Mestre):** Concluído e modularizado na pasta `12. Recursos do Mestre/` (6 arquivos: hub do capítulo/introdução/filosofia sandbox, complicações de arbitragem e condução de jogo com regras de comércio especulativo, conversão integral de materiais da 1ª Edição, compêndio de regras da casa opcionais, 10 matrizes culturais completas de geradores de nomes d100 com mais de 4.000 entradas, e 4 geradores de rolagem única para PNJs, patronos, encontros urbanos e encontros selvagens).
* **Ponto de Parada Atual:** Paramos no **Capítulo 13 (Campanhas Trans-humanas)**. As próximas tarefas de estruturação e revisão devem iniciar a partir deste capítulo.
